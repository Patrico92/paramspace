...
\comment{Theoretical suona troppo pomposo? Esportare di nuovo tutte le immagini libreoffice - svg - eps che viene meglio e aggiutare i nomi degli assi}
In this section we provide the concepts and the formal definitions that we will use later to express our algorithm. \comment{Ha un nome l'algoritmo?} First, we give the definitions of Pareto-set and Pareto-front that lie on the basis of Design Space Exploration is based on. Then, we sketch the fundamental idea behind our algorithm. Finally, we provide a formal definition of the regions, of their properties and of the operations that will be performed on them by the algorithm.
...
Let $\mathbf{c}',\mathbf{c}''\in S$ be two configurations, $b\in\mathcal{B}$ a benchmark application and $\mathbf{o}',\mathbf{o}''\in \Re^m$ the representation of the configurations in the objective space, i.e. $\mathbf{o}'=E \left(\mathbf{c}', b\right)$, $\mathbf{o}''=E \left(\mathbf{c}'', b\right)$, where $E$ is the evaluation function. The separation between

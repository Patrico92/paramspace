...
\comment{Attenzione: siamo capaci di avere a che fare con unfeasible configurations?}

\comment{Theoretical suona troppo pomposo? Esportare di nuovo tutte le immagini libreoffice - svg - eps che viene meglio, come si vede in \figR{3}. Aggiustare i nomi degli assi che in alcune figure sono sbagliati. O forse basta esportarli col nuovo libreoffice che fa meglio}
In this section we provide the concepts and the formal definitions that we will use later to express PS, the algorithm we propose.
 First, we give the definitions of Pareto-set and Pareto-front that lie on the basis of Design Space Exploration is based on. Then, we sketch the fundamental idea behind our algorithm. Finally, we provide a formal definition of the regions, of their properties and of the operations that will be performed on them by the algorithm.
...
\mathcal{C}^*(S)$ not dominated by any other \comment{feasible}
 configuration.
...
\comment{Mi sembra solo un cambio di notazione. }
...
and Pareto front (``objective space representation''). \comment{Questa figura sembra molto ``didattica''. La relazione tra Pareto front e Pareto set magari deve essere illustarta a studenti ma in un paper va bene? Io la apprezzerei se la trovassi, ma di solito si fa cosi'?}}
\end{figure}
...
\defL{separation}
Let $\mathbf{c}',\mathbf{c}''\in \mathcal{C}^*(S)$ be two feasible \comment{e' giusto che ho messo feasible? Se considerassimo anche le unfeasible sarebbe impossibile calcolarne le immagini nell'objective space?} configurations of the system $S$, $b\in\mathcal{B}$ a benchmark application and $\mathbf{o}',\mathbf{o}''\in \Re^m$ the representation of the configurations in the objective space, i.e. $\mathbf{o}'=E \left(\mathbf{c}', b\right)$, $\mathbf{o}''=E \left(\mathbf{c}'', b\right)$, where $E$ is the evaluation function. The separation between

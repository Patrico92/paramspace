...
when a significative number of configuration remain unaltered in the
...
different values of $K$ have been conducted, similar to the one shown in
Figure~\ref{fig:kimpact} which refer to the alloca\_test application
and a fixed amount of 500 configurations. We found that initially increasing values
of $K$ seem to improve both Pareto (as in the picture on the left) and objective
...
\comment{AA: Per rappresentare l'extent del Pereto front, forse
...
GA approaches. Again, it is worth pointing out how a definitive claim
about which algorithm shows better results, being that strictly
dependent on the design requirements and constraints, e.g.  whether we
need wide or dense results.
\comment{Da lasciare, da spostare ? da mostrare in modo semplice ?  non so}

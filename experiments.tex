...
\comment{AA: Da 'on the other side' fin qui non ho capito cosa vuoi dire}.
\comment{DAV: se non lo capisci vuol dire che o e' scritto male o e'
inutile, per adesso lo tolgo.}
...
\begin{figure}
  \figLC{kimpact}{Impact of increased K values on Pareto fronts and distribution for the alloca\_test application. \comment{AA: non capisco cosa sia la figura a destra. Inoltre, io vedo sulle ascisse e le ordinate di entrambi i grafici, le stesse grandezze (Time vs Power), quindi mi aspetto di trovare esattamente lo stesso plot sia a destra che a sinistra}}
  \begin{center}
...
To better evaluate results from a different perspective, we will
...
and GA for different benchmarks is shown in \figR{dispersion}(a) and
...
\begin{figure}
  ...
  different benchmarks and average dispersion absolute dispersion (c).
  \comment{AA: manca il titolo nella figura (c)} }
  \begin{center}
...
GA approaches. We highlight that PS shows a stronger \emph{uniform convergence} (discussed in \secL{metrics}), i.e. it provides a more extended Pareto front, avoiding the designer to neglect some important options, and offering a wider view of the potentiality of the designed device.

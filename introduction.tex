...
see also \defR{pareto-set}). The corresponding values of the
...
\figR{psos}). Of course this is only a first high-level system
...
in literature.  What makes the exploration of embedded systems so
complex is not only the huge size of the parameter spaces involved,
but also the tight interaction between parameters in relation to their
effect on the objective to be optimised, which take the form of
\emph{parameter dependencies} (see~\defR{dependency}). The 
...
space space exploration. The results of both works confirm the
important role that hardware/software dependencies can play in a
...
In fact, one of the purposes of the proposed PS algorithm is avoiding
the need of any a priori knowledge about those dependencies, letting
the algorithm itself discover regions where interaction between
...
not need this requirement and is the closest to ours, although the
...
non-dominated points will be considered in the next iteration. Our
approach is different. First, in the Sampling algorithm only the
...
points. Our algorithm avoids this, since even though a region has not
...
Many studies, as \cite{coello_easmop} and others, focus on genetic
approaches to solve multiobjective optimization problems. Genetic
approaches have many advantages: they provide good accuracy, they are
customizable and very general and require no a-priori knowledge to the
designer. The strong point of genetic approaches can be summarized as
follows: they consist in an exploration that is sufficiently broad
...
where authors specifically focuse on increasing the performance of a
evolutionaly exploration strategy, proposing a fuzzy based technique
to avoid the actual simulation of a certain number of configurations.
In particular, in this work we compare the proposed approach against a
well known Multi-Objective Genetic Algorithm~\cite{knowles_techrep06}
which is at best of our knowledge one of the most appropriate for the
particular scenario we adopted as testbed.  We claim that the approach
presented in this paper has most of the benefits of the genetic
